...
def generate_synthetic_financial_data(num_samples=1000, random_seed=RANDOM_SEED):
    ...
    data['subgroup_flag'] = np.random.choice(
        [0, 1], num_samples, p=[0.7, 0.3])  # e.g., market segment A vs B
    ...
    min_risk, max_risk = data['future_market_risk_score'].min(
    ), data['future_market_risk_score'].max()
    # Handle the case where min_risk equals max_risk to prevent division by zero
    if max_risk == min_risk:
        # Assign a neutral risk score if no variation
        data['future_market_risk_score'] = 0.5
    else:
        data['future_market_risk_score'] = (
            data['future_market_risk_score'] - min_risk) / (max_risk - min_risk)
...
def calculate_ece_from_regressor(y_true, y_pred, n_bins=10):
    ...
    # Use median for a balanced binary target
    risk_event_threshold = y_true.median()
    ...
    prob_true, prob_pred = calibration_curve(
        y_true_binary, y_pred_proba, n_bins=n_bins, strategy='uniform')
    ...
    for i in range(n_bins):
        ...
        if i == n_bins - 1:
            bin_indices = np.where(
                (y_pred_proba >= bins[i]) & (y_pred_proba <= bins[i+1]))
        else:
            bin_indices = np.where(
                (y_pred_proba >= bins[i]) & (y_pred_proba < bins[i+1]))
...
def evaluate_model_performance(model, X, y, subgroup_column=None):
    ...
    # Overall ECE
    # y_pred is a numpy array, y is a pandas Series, which calculate_ece_from_regressor handles
        ...
        if len(subgroups) > 1:
            ...
            for sg_value in subgroups:
                ...
                if sg_mask.sum() > 0:
                    ...
                    subgroup_y = y.loc[sg_mask.index[sg_mask]] if isinstance(
                        y, pd.Series) else y[sg_mask]
                    # Apply boolean mask to numpy array y_pred
                    subgroup_y_pred = y_pred[sg_mask.values]
                    subgroup_rmses[f'RMSE_Subgroup_{sg_value}'] = np.sqrt(
                        mean_squared_error(subgroup_y, subgroup_y_pred))
            ...
            if len(sorted_subgroup_keys) >= 2:
                delta = abs(
                    subgroup_rmses[sorted_subgroup_keys[0]] - subgroup_rmses[sorted_subgroup_keys[1]])
                subgroup_deltas['Subgroup_RMSE_Delta'] = delta
            else:
                # Not enough subgroups to compare
                subgroup_deltas['Subgroup_RMSE_Delta'] = 0.0
...
STRESS_TYPES = ['NOISE', 'FEATURE_SHIFT',
                'MISSINGNESS', 'OUT_OF_DISTRIBUTION', 'SUBGROUP']
...
def apply_noise(data: pd.DataFrame, feature: str, std_dev_multiplier: float, random_state: int) -> pd.DataFrame:
    ...
    data_copy[feature] = data_copy[feature] + \
        rng.normal(0, noise_magnitude, data_copy.shape[0])
    return data_copy
...
def apply_missingness(data: pd.DataFrame, feature: str, missing_percentage: float, random_state: int) -> pd.DataFrame:
    ...
    rng = np.random.RandomState(random_state)
    indices_to_na = rng.choice(
        data_copy.index, size=num_missing, replace=False)
...
def apply_out_of_distribution(data: pd.DataFrame, feature: str, scale_factor: float, base_value: float, random_state: int) -> pd.DataFrame:
    ...
    data_copy[feature] = base_value * \
        (1 + rng.uniform(scale_factor - 0.1,
         scale_factor + 0.1, data_copy.shape[0]))
    return data_copy
...
def execute_stress_scenario(model, scenario: dict, X_baseline: pd.DataFrame, y_baseline: pd.Series, subgroup_column: str, random_seed: int) -> dict:
    ...
    if scenario['stress_type'] == "NOISE":
        ...
        X_stressed[scenario['parameters']['feature']
                   ] = X_stressed[scenario['parameters']['feature']].fillna(imputation_value)
    ...
    stressed_metrics, _ = evaluate_model_performance(
        model, X_stressed, y_baseline, subgroup_column=subgroup_column)
...
ACCEPTANCE_THRESHOLDS = {
    # Max 10% *increase* in RMSE (i.e., degradation_pct must be >= -10)
    'RMSE': 10,
    # Max 5% *increase* in ECE (i.e., degradation_pct must be >= -5)
    'ECE': 5,
    # Max absolute delta increase in subgroup RMSE (not pct)
    'Subgroup_RMSE_Delta': 15
}
...
def calculate_degradation_and_status(
    ...
    for res in stressed_results:
        ...
        if baseline_value is None:
            print(
                f"Warning: Baseline value for {metric_name} not found. Skipping degradation calculation for this metric.")
            res.update({
                "baseline_value": None,
        ...
        degradation_pct = ((baseline_value - stressed_value) /
                           baseline_value) * 100 if baseline_value != 0 else float('inf')
        ...
        if threshold is not None:
            # Lower is better. Negative degradation_pct means stressed value is higher (worse).
            if metric_name in ['RMSE', 'ECE']:
                ...
                if degradation_pct < -threshold:  # If actual stressed value is more than X% higher than baseline
                    status = "FAIL"
            # Lower is better for delta. Compare absolute values.
            elif metric_name == 'Subgroup_RMSE_Delta':
                ...
                # If stressed delta is greater than baseline + tolerance
                if stressed_value > (baseline_value + threshold):
                    status = "FAIL"
                degradation_pct = ((stressed_value - baseline_value) / baseline_value) * \
                    100 if baseline_value != 0 else float(
                        'inf')  # Still calculate pct for display
...
#         y_baseline=global_y_test,
#         subgroup_column='subgroup_flag',
#         # Vary seed slightly for different noise patterns if applicable, but keep overall deterministic
...
def plot_metric_comparison(df: pd.DataFrame, metric_name: str):
    ...
    # Assumes baseline is consistent across scenarios for a metric
    baseline_value = metric_df['baseline_value'].iloc[0]
    ...
    plt.axhline(y=baseline_value, color='blue',
                linestyle='--', label='Baseline Value')

    plt.title(
        f'Baseline vs. Stressed {metric_name} Across Scenarios', fontsize=16)
...
def plot_degradation_percentages(df: pd.DataFrame, metric_name: str):
    ...
    plt.title(
        f'{metric_name} Degradation Percentage Across Scenarios', fontsize=16)
    plt.ylabel('Degradation Percentage (%)', fontsize=12)
    plt.xlabel('Real-World Event / Scenario', fontsize=12)
    plt.xticks(rotation=45, ha='right', fontsize=10)
    plt.yticks(fontsize=10)
    plt.legend(title='Validation Status', fontsize=10,
               title_fontsize=12)  # Add legend for status
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    return plt.gcf()
...
def plot_feature_distribution_shift(baseline_data: pd.DataFrame, stressed_data: pd.DataFrame, feature_name: str, scenario_description: str):
    ...
    sns.histplot(baseline_data[feature_name], kde=True,
                 color='blue', label='Baseline', stat='density', alpha=0.6)
    sns.histplot(stressed_data[feature_name], kde=True,
                 color='red', label='Stressed', stat='density', alpha=0.6)
    plt.title(
        f'Distribution Shift for {feature_name}\nUnder Scenario: {scenario_description}', fontsize=14)
...
# shift_scenario = next(
#     s for s in loaded_scenarios if s['stress_type'] == 'FEATURE_SHIFT')
# X_stressed_shift = global_X_test.copy()
# X_stressed_shift = apply_shift(
#     X_stressed_shift, shift_scenario['parameters']['feature'], shift_scenario['parameters']['shift_percentage'])
...
# noise_scenario = next(
#     s for s in loaded_scenarios if s['stress_type'] == 'NOISE')
...
# missingness_scenario = next(
#     s for s in loaded_scenarios if s['stress_type'] == 'MISSINGNESS')
...
# )
# X_stressed_missingness[missingness_scenario['parameters']['feature']
#                        ] = X_stressed_missingness[missingness_scenario['parameters']['feature']].fillna(imputation_value)
...
def generate_validation_decision(
    ...
    """
    total_scenarios = len(set(res['scenario_id']
                          for res in degradation_results))
    failed_metrics = [
        res for res in degradation_results if res['status'] == 'FAIL']
    ...
    if failed_metrics:
        ...
        recommended_actions.append(
            "Initiate model redesign to address identified vulnerabilities.")
        ...
        rationale.append(
            "The model passed all defined robustness acceptance thresholds under stress scenarios.")
        recommended_actions.append(
            "Proceed with model deployment, ensuring continuous monitoring for concept drift and data quality.")
...
| **Failed Tests** | {len(failed_metrics)} |
| **Pass Rate** | {((len(degradation_results) - len(failed_metrics)) / len(degradation_results) * 100):.1f}% |
...
> 
> The model has successfully passed all robustness and stress testing criteria.
"""
    elif decision == "RESTRICT":
        markdown_content += """
> **CONDITIONAL DEPLOYMENT - RESTRICTED USE**
> 
> The model has shown some vulnerabilities but may be deployed under specific constraints and enhanced monitoring.
"""
    else:  # REDESIGN
        markdown_content += """
...
"""
    ...
    if key_failures:
        ...
        markdown_content += "The following scenarios exceeded acceptable degradation thresholds:\n\n"
        markdown_content += "| Scenario | Metric | Baseline | Stressed | Degradation | Threshold | Status |\n"
        markdown_content += "|----------|--------|----------|----------|-------------|-----------|--------|\n"

        for fail in failed_metrics:
            markdown_content += f"| {fail['real_world_event'][:30]} | {fail['metric_name']} | {fail['baseline_value']:.4f} | {fail['stressed_value']:.4f} | {fail['degradation_pct']:.2f}% | {fail['threshold']}% | FAIL |\n"

        markdown_content += "\n### Detailed Failure Analysis\n\n"
        markdown_content += "\n".join(key_failures)
    else:
        markdown_content += "### All Tests Passed\n\n"
        markdown_content += "No critical failures identified. All metrics remained within acceptable degradation thresholds across all stress scenarios.\n"
...
"""
    ...
    threshold_descriptions = {
        'RMSE': 'Maximum acceptable % increase in Root Mean Squared Error',
        'ECE': 'Maximum acceptable % increase in Expected Calibration Error',
        'Subgroup_RMSE_Delta': 'Maximum acceptable absolute increase in subgroup performance disparity'
    }

    for metric, threshold in acceptance_thresholds.items():
        desc = threshold_descriptions.get(metric, 'Custom metric threshold')
        unit = "% increase" if metric != 'Subgroup_RMSE_Delta' else "absolute units"
        markdown_content += f"| **{metric}** | {threshold} {unit} | {desc} |\n"
...
"""
    ...
    print(
        f"Validation decision report generated and saved to {output_filepath}")
